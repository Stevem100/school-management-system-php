...
$branch = $branch ?? [];
$schools = $schools ?? [];
$schoolId = $schoolId ?? ($branch['school_id'] ?? '');
$title = 'Create Branch';
$formAction = url('/branches');
?>
...
<div class="max-w-2xl mx-auto space-y-6">
  ...
  <div class="flex items-center gap-4">
    ...
    <div>
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Add a new branch to a school</p>
    </div>
  </div>
  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
    <form method="POST" action="<?= $formAction ?>">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select id="school_id" name="school_id" required class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
              ...
              <option value="<?= e($s['id']) ?>" <?= ($schoolId == $s['id']) ? 'selected' : '' ?>><?= e($s['name']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div>
            ...
            <input type="text" id="name" name="name" required value="<?= e($branch['name'] ?? '') ?>" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent" placeholder="e.g. Main Campus">
          </div>
        </div>
      ...
      <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-3">
        ...
        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">Create Branch</button>
      </div>
    </form>
  </div>
</div>
