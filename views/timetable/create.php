...
$periods = [];
for ($i = 1; $i <= 10; $i++) $periods[] = "Period $i (0" . ($i + 6) . ":00 - 0" . ($i + 7) . ":00)";
...
<div class="max-w-2xl mx-auto space-y-6">
  <div class="flex items-center gap-4">
    ...
    <div>
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Add a slot to the timetable</p>
    </div>
  </div>
  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
    <form method="POST" action="<?= $formAction ?>">
      ...
      <div class="p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select id="teacher_id" name="teacher_id" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
              ...
              <option value="<?= e($t['id']) ?>"><?= e(trim(($t['firstName'] ?? '') . ' ' . ($t['lastName'] ?? ''))) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="time" id="start_time" name="start_time" value="<?= e($slot['start_time'] ?? '08:00') ?>" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
          </div>
          <div>
            <label for="end_time" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">End Time</label>
            <input type="time" id="end_time" name="end_time" value="<?= e($slot['end_time'] ?? '09:00') ?>" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
          </div>
        </div>
        <div>
      ...
      <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-3">
        ...
        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">Create Slot</button>
      </div>
    </form>
  </div>
</div>
