...
$teachers = $teachers ?? [];
$title = 'Edit Class';
$formAction = url('/classes/' . ($class['id'] ?? ''));
...
<div class="max-w-2xl mx-auto space-y-6">
  <div class="flex items-center gap-4">
    ...
    <div>
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Update class details below</p>
    </div>
  </div>
    ...
    <form method="POST" action="<?= $formAction ?>">
      ...
      <div class="p-6 space-y-5">
        <div>
          ...
          <input type="text" id="name" name="name" required value="<?= e($class['name'] ?? '') ?>" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select id="grade_level" name="grade_level" required class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
              ...
              <option value="<?= e($gl) ?>" <?= ($class['gradeLevel'] ?? '') === $gl ? 'selected' : '' ?>><?= e($gl) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div>
            ...
            <input type="text" id="section" name="section" required value="<?= e($class['section'] ?? '') ?>" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
          </div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" id="capacity" name="capacity" required min="1" value="<?= e($class['capacity'] ?? 40) ?>" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
          </div>
          <div>
            ...
            <select id="academic_year" name="academic_year" required class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
              ...
              <option value="<?= e($ay) ?>" <?= ($class['academicYear'] ?? '') === $ay ? 'selected' : '' ?>><?= e($ay) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        <div>
          ...
          <select id="class_teacher_id" name="class_teacher_id" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
            ...
            <option value="<?= e($t['id'] ?? '') ?>" <?= ($class['classTeacherId'] ?? '') == $t['id'] ? 'selected' : '' ?>><?= e(trim(($t['firstName'] ?? '') . ' ' . ($t['lastName'] ?? ''))) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div>
      ...
      <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-3">
        ...
        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">Update Class</button>
      </div>
    </form>
  </div>
</div>
