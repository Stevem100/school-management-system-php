...
$exams = $exams ?? [];
$title = 'Edit Result';
$formAction = url('/results/' . ($result['id'] ?? ''));
?>
...
<div class="max-w-2xl mx-auto space-y-6">
  <div class="flex items-center gap-4">
    ...
    <div>
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Update result details below</p>
    </div>
  </div>
  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
    <form method="POST" action="<?= $formAction ?>">
      ...
      <div class="p-6 space-y-5">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select id="student_id" name="student_id" required class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
              ...
              <option value="<?= e($s['id']) ?>" <?= ($result['student_id'] ?? '') == $s['id'] ? 'selected' : '' ?>><?= e(($s['first_name'] ?? '') . ' ' . ($s['last_name'] ?? '')) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div>
            ...
            <select id="exam_id" name="exam_id" required class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
              ...
              <option value="<?= e($ex['id']) ?>" <?= ($result['exam_id'] ?? '') == $ex['id'] ? 'selected' : '' ?>><?= e($ex['name'] ?? '') ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" id="marks_obtained" name="marks_obtained" required min="0" value="<?= e($result['marks_obtained'] ?? '') ?>" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
          </div>
          <div>
        ...
        <div>
          ...
          <textarea id="remarks" name="remarks" rows="2" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none"><?= e($result['remarks'] ?? '') ?></textarea>
        </div>
      </div>
      <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-3">
        ...
        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">Update Result</button>
      </div>
    </form>
  </div>
</div>
